namespace Nodis.Core.Threading;
...
public class LimitedConcurrencyLevelTaskScheduler : TaskScheduler
{
    ...
    private readonly LinkedList<Task> tasks = []; // protected by lock(_tasks)
    ...
    protected sealed override bool TryExecuteTaskInline(Task task, bool taskWasPreviouslyQueued)
    {
        ...
        // If the task was previously queued, remove it from the queue, then try to run the task
        if (taskWasPreviouslyQueued) return TryDequeue(task) && TryExecuteTask(task);
